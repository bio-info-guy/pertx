    ...
    def __init__(
        ...
        if self.gene_emb_style == 'vanilla':
            ...
            self.base_emb = GeneHybridEmbedding(
                ...
                dim = self.gene_emb_dim,
                padding_idx=self.pad_id
            )
            self.encoder = GeneHead(self.base_emb, self.d_model) if self.gene_emb_dim != self.d_model else self.base_emb
        elif self.gene_emb_style == 'multi-hybrid':
        ...
        if cross_attn_decoder:
            self.cross_attn = CrossAttn(self.d_model, self.nhead, self.dropout)
        self.init_weights()
