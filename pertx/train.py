...
from .tokenizer.custom_tokenizer import tokenize_and_pad_batch, random_mask_value
from .utils.logger import create_logger
from .optim.optimizer import create_optimizer_dict
from .utils.plot import process_and_log_umaps
from .utils.misc import init_plot_worker
from .optim.loss import (
    compute_hard_negative_link_loss, 
    compute_margin_link_loss, 
    cce_loss, 
...
def wrapper_train(model, config, data_gen,
    ...
    logger = create_logger() if logger is None else logger
    if config.get('link_loss', None) == 'hard':
        print('hard negative link loss is on')
    if device is None:
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    all_edges = None
    if model.pert_graph is not None:
        all_edges = extract_master_edge_index(model.pert_encoder, to_cpu=True)
    ...
    if config.get('ctrl_input_only', False):
        new_train_inds = list(set(train_loader.dataset.indices).intersection(np.where(train_loader.dataset.adata.obs.genotype == 'WT')[0]))
        new_valid_inds = list(set(valid_loader.dataset.indices).intersection(np.where(valid_loader.dataset.adata.obs.genotype == 'WT')[0]))
        train_loader.dataset.set_new_indices(new_train_inds , False)
        valid_loader.dataset.set_new_indices(new_valid_inds, False)

    optimizer_dict = create_optimizer_dict(model, device, config, dataloader=train_loader, num_batch_types=num_batch_types)
    
    ctrl_pert_index = data_gen["genotype_to_index"]['WT']
